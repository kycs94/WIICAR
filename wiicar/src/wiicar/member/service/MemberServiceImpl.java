...
public class MemberServiceImpl implements MemberService {
	...
	public Map<String, Object> getMyProfile() throws SQLException {
		...
		String[] driverReviewId = new String[driverReviewCount];
		String[] driverReviewContent = new String[driverReviewCount];
		for(int i = 0; i < driverReviewCount; i++) {
			MemberDTO driver = carpoolDAO.getMember(driverReview.get(i).getId());
			driverReviewImgs[i] = driver.getProfileImage();
			driverReviewId[i] = driver.getNickname();
			driverReviewContent[i] = driverReview.get(i).getContent();
		}
		...
		String[] passengerReviewId = new String[passengerReviewCount];
		String[] passengerReviewContent = new String[passengerReviewCount];
		for(int i = 0; i < passengerReviewCount; i++) {
			MemberDTO passenger = carpoolDAO.getMember(passengerReview.get(i).getId());
			passengerReviewImgs[i] = memberDAO.getMemberInfo(passengerReview.get(i).getId()).getProfileImage();
			passengerReviewId[i] = passenger.getNickname();
			passengerReviewContent[i] = passengerReview.get(i).getContent();
		}
		...
		result.put("driverReviewId", driverReviewId);
		result.put("driverReviewContent", driverReviewContent);
		...
		result.put("passengerReviewId", passengerReviewId);
		result.put("passengerReviewContent", passengerReviewContent);
	...
	@Override
	   ...
	   public Map<String, Object> getPassangersCarpoolList(String sort) throws SQLException {
	      ...
	      String id = (String)RequestContextHolder.getRequestAttributes().getAttribute("sid", RequestAttributes.SCOPE_SESSION);
	      List<ReservationsDTO> reservationList = null;
	      List<CarpoolDTO> carpoolList = new ArrayList<CarpoolDTO>();     
	      ...
	      int[] reviewCheck = new int[listSize];
	      CarpoolDTO cdto = new CarpoolDTO();
	      for(int i = 0; i < listSize; i++) {
	         carpoolNum[i] = (reservationList.get(i).getCarpoolnum());
	         cdto.setCarpoolNum(carpoolNum[i]);
	         carpoolList.add(carpoolDAO.getCarpool(cdto));
	      }
	      ...
	      for(int i = 0; i < listSize; i++) {
	         ...
	         if((driverdto = carpoolDAO.getMember(dto.getDriverId())) != null) {
	            ...
	            passengers = carpoolDAO.getPassengers(dto.getCarpoolNum());
	            passengercount[i] = passengers.size();
	            reviewCheck[i] = memberDAO.getReviewCheck(dto.getCarpoolNum(), id, 0);
	            if(passengers.size() == 0) {
	               ids = null;
	               imgs = null;
	            } else if(passengers.size() != 0) {
